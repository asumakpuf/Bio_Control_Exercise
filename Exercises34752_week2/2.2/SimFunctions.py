...
class SimulationFunctions:
    ...
    def fkinematics(self, ang):
        elbow_pos = np.array([self.le1 * np.cos(ang[0]), self.le1 * np.sin(ang[0])])
        rotation_matrix = np.array([[np.cos(ang[0]), -np.sin(ang[0])], [np.sin(ang[0]), np.cos(ang[0])]])
        wrist_vector = np.array([self.le2 * np.cos(ang[1]), self.le2 * np.sin(ang[1])])
        wrist_pos = elbow_pos + rotation_matrix @ wrist_vector
